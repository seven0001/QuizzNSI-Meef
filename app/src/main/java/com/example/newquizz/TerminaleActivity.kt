...
class TerminaleActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        questionNumber = findViewById<TextView>(R.id.terminale_question_number);
        questionText = findViewById<TextView>(R.id.terminale_question_text);
        scoreText = findViewById<TextView>(R.id.terminale_score)

        // Initialisation des boutons de réponse
        answersButtons = arrayOf(
            findViewById<Button>(R.id.terminale_answer1),
            findViewById<Button>(R.id.terminale_answer2),
            findViewById<Button>(R.id.terminale_answer3),
            findViewById<Button>(R.id.terminale_answer4),
        )

        // Chargement des questions depuis la base de données
        GlobalScope.launch {
            loadQuestionsTerminale()
        }
    ...
    private fun loadQuestionsTerminale() {
        val service = QuestionService();

        GlobalScope.launch {
            val allQuestions = service.getQuestions(2); // Récupération des questions de la catégorie 2
    ...
    private fun displayScore() {
        scoreText.setText("Score : $currentScore/5") // Affichage du score
    }
}
